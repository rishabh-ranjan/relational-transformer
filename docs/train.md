...
laid out, and [`expts/pretrain/submit_ilc.py`](../expts/pretrain/submit_ilc.py)
for a worked one -- it passes `rt.train:main` straight to `submit`, which is the
least boilerplate a run on this cluster can be.
