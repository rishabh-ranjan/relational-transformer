...
arms. The evals themselves live in [`../repaper_scaling`](../repaper_scaling).

## Running it, in order

```bash
# 0. one-time fetches, on the login node (compute nodes have no internet)
pixi run python -m expts.repaper_baselines.fetch_tabicl
...
RELBENCH_CACHE_DIR=/dfs/user/ranjanr/share/relational-transformer/repaper/relbench-cache \
    pixi run -e featurize python -m expts.repaper_baselines.check_alignment

# 3. featurize (edit the __main__ block to pick stages), then the indices
pixi run python expts/repaper_baselines/submit.py
```
...
Logs land under
`/dfs/user/ranjanr/slurm-logs/rishabh-ranjan/relational-transformer/expts/repaper_baselines`.
...
`abl/vdb_*` arms in `../repaper_scaling`) rebuild the extension in their
clone's setup step:
