...
validation score (from [`../repaper_tune`](../repaper_tune)) each run with
...
```bash
pixi run python -m expts.repaper_submit.submit   # 84 one-GPU units (21 tasks x 4 cfgs)
pixi run python -m expts.repaper_submit.reduce   # -> CSVs + results.json; prints next step
...
A unit is `expts.repaper_enscurve.run:main` (the ensemble runner) at
