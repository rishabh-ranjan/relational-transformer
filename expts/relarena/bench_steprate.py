...
def main(dataset: str, task: str, cache_dir: str, steps: int = 300) -> None:
    ...
    from relarena.models.rt.export import TASK_DIR, preprocessed_dir
    ...
    pre = preprocessed_dir(
        ...
        identity=src.run_identity("inner"),
        db_name=DB_NAME,
    )
    ...
    def run(label: str, **over) -> float:
        args = cfg.train_args(
            ...
            train_split="train",
            eval_split="val",
            db_cutoff=cfg.context_cutoff(src.task, "val"),
            total_steps=steps,
            out_root=f"/tmp/ranjanr/bench-{label}",
            run_id=label,
            seed=0,
        )
        ...
        print(
            f"RATE {label:22s} {dt:7.1f}s for {steps} steps -> {dt / steps:5.3f} s/step",
            flush=True,
        )
        return dt
    ...
    print("\n=== attribution")
    print(
        f"  in-loop eval share : {(base - noev) / base * 100:5.1f}%  ({base - noev:.0f}s of {base:.0f}s)"
    )
    print(
        f"  walks 10k -> 1k    : {(noev - w1k) / noev * 100:5.1f}% faster ({noev:.0f}s -> {w1k:.0f}s)"
    )
    print(
        f"  walks 10k -> 0     : {(noev - w0) / noev * 100:5.1f}% faster ({noev:.0f}s -> {w0:.0f}s)"
    )
    print(
        f"  pure train s/step  : {noev / steps:.3f} (walks 10k), {w1k / steps:.3f} (1k), {w0 / steps:.3f} (0)"
    )
