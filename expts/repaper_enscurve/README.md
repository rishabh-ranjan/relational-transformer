...
tuned context (from [`../repaper_tune`](../repaper_tune)); plus its per-task
appendix figures.

## Running it

```bash
pixi run python -m expts.repaper_enscurve.submit    # 21 jobs per variant, 1 GPU each
pixi run python -m expts.repaper_enscurve.reduce    # -> wandb runs `default`, `tuned`
```

The tuned variant waits on `../repaper_tune/tuned_configs.json`; the default
