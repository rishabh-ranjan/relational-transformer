"""Task resolution for stage 2, independent of the ``rt`` package.

Stage 2 runs in its own pixi env (rdblearn needs Python 3.12 and a conflicting
relbench), so it cannot import ``rt``: ``rt.data.__init__`` pulls
``rt.data.datasets``, which needs ``ml_dtypes``, torch and the compiled
``rt.rustler`` extension -- none of which belong in a CPU-only DFS environment.

So the two pieces stage 2 needs are vendored here, copied from
``src/rt/data/tasks.py`` and ``src/rt/data/resolve.py``. ``eval.py`` checks this copy
against the real ``rt.data.get_tasks`` at startup and fails loudly if they disagree,
...
def read_meta(pre_dir, db: str) -> dict:
    """Read one preprocessed dataset's meta.json (verbatim from rt.data.resolve)."""
