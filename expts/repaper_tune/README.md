...
(`../repaper_valtest`), and the top-4-configs leaderboard ensemble
(`../repaper_submit`).

## Running it

```bash
pixi run python -m expts.repaper_tune.submit     # 21 tuning jobs, 1 GPU each
pixi run python -m expts.repaper_tune.collect    # -> tuned_configs.json; commit it
```
