...
# Sweep 2 of the augmented arm. Identical to submit_val.py -- same predictor,
# same 2**18 in-sample context, same val split, same seeds -- except that the
# features come from blobs rt-j produced while the numeric augmentation was
# injecting derived cells into its input. The two rounds differ in exactly one
# thing, the feature root, so the comparison is clean.
#
# Sweep 1 (submit_featurize_aug.py) must have written a db's blob before its
# tasks can run here; blob_ready() below is the gate.
ROUND = "rtj_val_aug"
    ...
    submit(
        ...
        args=dict(
            ...
            tags=dict(
                ...
                augment_transforms="ecdf+signed_log1p+pair_product",
