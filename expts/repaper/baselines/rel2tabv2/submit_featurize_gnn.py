...
for db in DBS:
    ...
    submit(
        ...
        args=dict(
            ...
            pre_dir=PRE_DIR,
            raw_dir=RAW_DIR,
            features_root=f"{SHARE}/features_gnn",
            graph_cache_dir=f"{SHARE}/gnn-graph-cache",
            text_embedder_dir=f"{SHARE}/glove",
            # The example's defaults: channels 128, 2 layers, num_neighbors 128
            # halved per hop -> [128, 64]. num_layers=2 is the 2-hop depth that
            # matches rdblearn's DFSConfig(max_depth=2).
            channels=128,
            num_layers=2,
            num_neighbors=128,
            aggr="sum",
