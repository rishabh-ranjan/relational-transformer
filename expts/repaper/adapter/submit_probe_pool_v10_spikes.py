...
submit(
    ...
    args=dict(
        ...
        ckpt_dir=f"{OUT_ROOT}/adapter/{RUN}/spikes",
        out_dir=f"{LOG_ROOT}/repaper/adapter/probe-pool-v11-exact",
        spikes=[
            dict(step=871, task="join-se-economics/user-comment-score-sum-heavytail", ckpt_step=871, logged_grad_norm=941.1, logged_loss=-5.1393),
        ],
        # spikes=[
        #     dict(step=797, task="join-se-buddhism/user-comment-score-mean-heavytail", ckpt_step=800, logged_grad_norm=1046.0, logged_loss=-5.2593),
        #     dict(step=607, task="join-apple-podcasts/podcast-review-count", ckpt_step=600, logged_grad_norm=318.7, logged_loss=-3.9038),
        #     dict(step=318, task="join-nhtsa-fars/persons-alc_det", ckpt_step=300, logged_grad_norm=312.3, logged_loss=-4.6197),
        #     dict(step=216, task="join-se-boardgames/user-post-score-mean-heavytail", ckpt_step=200, logged_grad_norm=281.6, logged_loss=-5.2406),
        #     dict(step=731, task="join-se-anime/user-post-score-max-heavytail", ckpt_step=750, logged_grad_norm=149.7, logged_loss=-5.0615),
        # ],
        # spikes=[
    ...
    name="adapter-probe-pool-v11-exact",
    repo_root=REPO_ROOT,
    cluster=ILC,
