"""Ensemble the fine-tuned checkpoints over context seeds on test, one job per
task. No context tuning: the grid is the single config the fine-tuning runs
evaluated with, so no val split is read. See [README.md](README.md).

`rt.eval` scores the running average after every seed, so one job yields the
whole test-metric-vs-ensemble-size curve.
"""
...
def ckpt_for(db: str, task: str) -> str:
    ...
    those instead to ensemble one net's weights rather than the winner's.
    """
    runs = sorted(
        d
...
def plan(n: int) -> list[Resources]:
    ...
    Blackwell throughout: blackwell1 has the cards free, and this is 16 test
    passes back to back, so the faster card is the whole wall clock.
