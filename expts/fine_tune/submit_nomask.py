...
def plan(n: int) -> list[Resources]:
    ...
    blackwell1's idle cards first -- a run is ~20 minutes there, so an `il-lo`
    b200 turns over faster than a queued slot on a capped tier -- then amperes
    on `il-lo`, which is uncapped. The un-preemptible tiers are full: `il`'s
    10-gpu ceiling is spent on the pretrain job and `il-interactive`'s 2 are
    held by this user's other arms.
    ...
    out = [a100("il", "7-00:00:00")] * min(n, 1)
    ...
    for (db, task), resources in zip(tasks, plan(len(tasks)), strict=True):
        ...
        submit(
            ...
            args=dict(
                ...
                eval_lcs_bw_pl_grid=[(1024, 64, True)],
                targets=targets_for(db, task),
                project="2026-08-07-fine_tune",
                entity="rtv2",
                run_name=f"{name} nomask",
                wandb_disabled=False,
                out_root="/dfs/user/ranjanr/ckpts",
            ),
            resources=resources,
            name=f"{db}-{task}-nomask",
