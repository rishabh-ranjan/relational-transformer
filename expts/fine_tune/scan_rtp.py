"""Pretrained arm against its baseline, per task, from wandb.

    pixi run python expts/fine_tune/scan_rtp.py

Two rows per task: the run warm-started from a checkpoint (`load_ckpt_path` set
-- RT-P), and the run of the same task that started from random init
(`load_ckpt_path=None`).
...
def confounds_of(run, ref, task) -> set[str]:
    ...
    `load_ckpt_path` is the difference under test. `IGNORED` is the set that
...
def main() -> None:
    ...
    for run in runs:
        ...
        if run.config.get("load_ckpt_path") is not None:
            if task not in warm or run.created_at > warm[task].created_at:
                warm[task] = run
        else:
            base.setdefault(task, []).append(run)

    for task in sorted(warm):
        db, name = task
        ...
        if not candidates:
            print(f"{db}/{name}: no random-init run to compare against\n")
            continue
        fewest = min(len(confounds_of(warm[task], r, task)) for r in candidates)
        ref = max(
            (r for r in candidates if len(confounds_of(warm[task], r, task)) == fewest),
        ...
        metric_and_dir = [
            ...
            if f"{m}/val/{db}/{name}" in warm[task].summary
        ]
        if not metric_and_dir:
            print(f"{db}/{name}: the pretrained run has logged no val metric yet\n")
            continue
        metric, higher = metric_and_dir[0]
        horizon = min(warm[task].config["total_steps"], ref.config["total_steps"])
        ...
        for kind, prefix in (("live", ""), ("swa", "swa/")):
            ...
            for label, run in (
                ("load_ckpt_path=rt-p", warm[task]),
                ("load_ckpt_path=None", ref),
            ):
        ...
        confounds = sorted(confounds_of(warm[task], ref, task))
        print(
            "  effectively differs only in load_ckpt_path"
            if not confounds
