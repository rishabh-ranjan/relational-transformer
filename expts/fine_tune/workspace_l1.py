"""Build the saved view that compares l1 against huber on the four regression
tasks `submit_l1.py` runs.

    pixi run python expts/fine_tune/workspace_l1.py
...
(`submit_l1.loss_fn="l1"` against `submit.loss_fn="huber"`), and everything
...
- the runset draws only the eight runs of the comparison and groups on
...
- panels for the other benchmark tasks come out, and so does the val split.
...
These tasks are scored by nMAE, where lower is better and the target line is a
ceiling to get under rather than a bar to clear.
"""
...
# link that opens it. See `workspace_stack.SLUG` for the shape and why it is
# alphanumeric; the assert in `main` is what holds it.
SLUG = "l1vshuber"

# The eight runs of the comparison, by the `run_name` config `submit` and
# `submit_l1` set: `{db}/{task}-train`, with an `-l1` suffix on the l1 arm.
...
RUNS = [f"{db}/{task}-train{suffix}" for db, task in TASKS for suffix in ("", "-l1")]

PANEL_TASKS = frozenset(f"{db}/{task}" for db, task in TASKS)
...
def keeps(title: str) -> bool:
    ...
    return task not in workspace.sizes() or task in PANEL_TASKS
...
def main() -> None:
    ...
    sections = []
    for s in w.sections:
        # The val split is the arm's own model selection, not a comparison, and
        # beside the test panels it reads as one.
        if s.name == "dashboard: nmae/val":
            continue
