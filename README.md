...
Four kinds of checkpoints are provided:
...
- `leaderboard-finetuned/finetune-from-{pretrain,contd-pretrain}_<dataset>_<task>.pt`: fine-tuned checkpoints behind the replicated "RT | pretrained + fine-tuned" cells on the [RelBench leaderboard](https://relbench.stanford.edu/leaderboard/). The board reports the per-task best over fine-tuning from the plain-pretraining vs continued-pretraining init; the file provided is the winning init for that task. Regression fine-tuning uses val-NMAE (MAE / train-split std) best-checkpoint selection, matching the leaderboard metric; numbers are full official test split. Cells without a file here are the paper's own pretrain-init fine-tuning numbers (reproduce via `scripts/example_finetune.py`).
- rel-event checkpoints (`pretrain_rel-event_<task>.pt`, `contd-pretrain_rel-event_<task>.pt`): rel-event was not covered by the original release; these produce the leaderboard's RT rel-event cells.
